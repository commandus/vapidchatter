...
bool WpnKeys::isPrivateKeyValid(
	const std::string &value
)
{
  uint8_t privateKey[ECE_WEBPUSH_PRIVATE_KEY_LENGTH];
	return ece_base64url_decode(value.c_str(), value.size(), ECE_BASE64URL_REJECT_PADDING, privateKey, ECE_WEBPUSH_PRIVATE_KEY_LENGTH) == ECE_WEBPUSH_PRIVATE_KEY_LENGTH;
}

bool WpnKeys::isPublicKeyValid(
	const std::string &value
)
{
	uint8_t publicKey[ECE_WEBPUSH_PUBLIC_KEY_LENGTH];
	return ece_base64url_decode(value.c_str(), value.size(), ECE_BASE64URL_REJECT_PADDING, publicKey, ECE_WEBPUSH_PUBLIC_KEY_LENGTH) == ECE_WEBPUSH_PUBLIC_KEY_LENGTH;
}

bool WpnKeys::isAuthSecretValid(
	const std::string &value
)
{
	uint8_t authSecret[ECE_WEBPUSH_AUTH_SECRET_LENGTH];
	return ece_base64url_decode(value.c_str(), value.size(), ECE_BASE64URL_REJECT_PADDING, authSecret, ECE_WEBPUSH_AUTH_SECRET_LENGTH) == ECE_WEBPUSH_AUTH_SECRET_LENGTH;
}
...
bool Subscription::isTokenValid(const std::string &subscriptionToken)
{
...
{
	std::string key;
	searchSubscriptionPublicKey(const std::string & v) : key(v) {}
	bool operator()(const Subscription &m)
	{
		return m.getWpnKeys().getPublicKey() == key;
	}
};


struct searchSubscriptionTokenNAuth
{
	std::string token;
  std::string auth;
	searchSubscriptionTokenNAuth(const std::string &subscriiptionToken, const std::string &authSecret) 
    : token(subscriiptionToken), auth(authSecret) {}
	bool operator()(const Subscription &m)
	{
		return (m.getWpnKeys().getAuthSecret() == auth) && (m.getToken() == token);
	}
};
...
Subscription *Subscriptions::findByTokenNAuthSecret(
  const std::string &subscriptionToken,
  const std::string &authSecret) const
{
	if (authSecret.empty())
		return NULL;
	std::vector<Subscription>::const_iterator it = std::find_if(list.begin(), list.end(), searchSubscriptionTokenNAuth(subscriptionToken, authSecret));
	if (it == list.end())
		return NULL;
	else
		return (Subscription *) &(*it);
...
bool Subscriptions::rmByPublicKey(const std::string &key) {
	std::vector<Subscription>::iterator it = std::find_if(list.begin(), list.end(), searchSubscriptionPublicKey(key));
	if (it == list.end())
		return false;
	list.erase(it);
	return true;
...
void ConfigFile::load(
	  std::istream &configRead,
	  bool isJson
) {
	if (configRead.fail()) {
		errorCode = ERR_CONFIG_FILE_READ;
		errorDescription = "Error read config " + fileName;
	} else {
		reset();
		if (isJson) {
#ifdef USE_JSON_NLOHMANN
...
ConfigFile::ConfigFile(
  ...
  bool isJson
	...
	: outputFormatCode(0), errorCode(0), errorDescription(""), fileName(".js"), clientOptions(NULL), androidCredentials(NULL), wpnKeys(NULL), subscriptions(NULL)
{
  load(strm, isJson);
}
	...
	: outputFormatCode(0), errorCode(0), errorDescription(""), fileName(filename), clientOptions(NULL), androidCredentials(NULL), wpnKeys(NULL), subscriptions(NULL)
{
	...
	load(configRead, filename.find(".js") != std::string::npos);
	configRead.close();
}
