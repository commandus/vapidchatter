...
)
{
	...
	if (config->androidCredentials->getAndroidId() == 0)
	{
      ...
      errorDescription = std::string(getWpnErrorDescription(errorCode, 0)) + e + toString(r);
			return false;
		}
...
) 
{
    ...
    errorCode = ERR_SUBSCRIPTION_NOT_FOUND;
		errorDescription = getWpnErrorDescription(errorCode, 0);
		return c;
	}
	std::string retval;
