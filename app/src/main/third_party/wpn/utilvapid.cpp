...
)
{
  ...
  if (rawRecvPubKeyLen == 0)
    return ECE_ERROR_INVALID_PUBLIC_KEY;
	
	uint8_t authSecret[ECE_WEBPUSH_AUTH_SECRET_LENGTH];
	size_t authSecretLen = ece_base64url_decode(auth.c_str(), auth.size(), ECE_BASE64URL_REJECT_PADDING, authSecret, ECE_WEBPUSH_AUTH_SECRET_LENGTH);
  if (authSecretLen == 0)
    return ECE_ERROR_INVALID_AUTH_SECRET;
...
#ifdef DEBUG
	std::cerr <<  "p256dh: " << p256dh << std::endl
	<<  ", auth: " << auth << std::endl
	<<  ", body: " << body << std::endl
	<<  ", cipher len: " << cipherString.size() << std::endl
	<<  ", cipher (hex): " << hexString(cipherString) << std::endl;
#endif
	if (expiration == 0)
		expiration = time(NULL) + (12 * 60 * 60);
...
)
{
	...
	std::string jsonMsg = jsClientNotification(subscriptionJSON, subject, body, icon, link, badge, sound, data);
	return webpushVapid(reuseCurl, retval, publicKey, privateKey, endpoint, p256dh, auth, jsonMsg, 
		contact, contentEncoding, expiration);
}
