...
static const char* JSON_NOTIFICATION_BADGE = "badge";
...
static const char* JSON_NOTIFICATION_EXTRA_DATA = "extra_data";
static const char* JSON_NOTIFICATION_DATA = "data";

static const char* JSON_NOTIFICATION_DATA_ON_ACTION_CLICK = "onActionClick";
static const char* JSON_NOTIFICATION_ACTIONS = "actions";
static const char* JSON_NOTIFICATION_ACTION = "action";
static const char* JSON_NOTIFICATION_DATA_ACTION_DEFAULT = "default";
static const char* JSON_NOTIFICATION_DATA_ACTION_OPERATION = "operation";
static const char* JSON_NOTIFICATION_DATA_ACTION_URL = "url";
...
std::string notify2json(
	...
	RAPIDJSON_ADD_PCHAR(n, v, a, extra_data, notify->data)
	// new data is object
	Value oac;
	oac.SetObject();
	Value defaultAction;
	defaultAction.SetObject();
	RAPIDJSON_ADD_PCHAR(defaultAction, v, a, operation, "openWindow")
	RAPIDJSON_ADD_PCHAR(defaultAction, v, a, url, notify->link)

	oac.AddMember("default", defaultAction, a);
	dat.AddMember("onActionClick", oac, a);
	n.AddMember("data", dat, a);
...
)
{
	...
	RAPIDJSON_ADD_STRING(n, v, a, badge, badge)
	RAPIDJSON_ADD_STRING(n, v, a, sound, sound)
	RAPIDJSON_ADD_STRING(n, v, a, click_action, link)
	// deprecated
	if (!data.empty()) {
		RAPIDJSON_ADD_STRING(n, v, a, extra_ata, data)
	}
	/*
	...
	*/
	...
	defaultAction.SetObject();
	RAPIDJSON_ADD_PCHAR(defaultAction, v, a, operation, "openWindow")
	RAPIDJSON_ADD_STRING(defaultAction, v, a, url, link)
...
)
{
	...
	if (n.IsObject()) {
		...
		if (n.HasMember("badge")) {
			Value &v = n["badge"];
			if (v.IsString())
				badge = v.GetString();
		}
		if (n.HasMember("sound")) {
			Value &v = n["sound"];
			if (v.IsString())
				sound = v.GetString();
		}
		if (n.HasMember("click_action")) {
			Value &v = n["click_action"];
			if (v.IsString())
				click_action = v.GetString();
		}
		if (n.HasMember("data")) {
			Value &dat = n["data"];
			if (dat.IsString())
				...
				if (dat.IsObject()) {
					if (dat.HasMember("onActionClick")) {
						Value &oac = dat["onActionClick"];
						if (oac.IsObject()) {
							if (oac.HasMember("default")) {
								Value &defaultaction = oac["default"];
								if (defaultaction.IsObject()) {
									if (defaultaction.HasMember("url")) {
										Value &url = defaultaction["url"];
										if (url.IsString()) {
											click_action = url.GetString();
										}
									}
								}
							}
        ...
        { JSON_NOTIFICATION, 
            {
                ...
                { JSON_NOTIFICATION_EXTRA_DATA, notify->data}, // deprecated
				{ JSON_NOTIFICATION_DATA, // new one is object
					{ JSON_NOTIFICATION_DATA_ACTION_DEFAULT,
						{
							{ JSON_NOTIFICATION_DATA_ACTION_OPERATION, "openWindow" },
							{ JSON_NOTIFICATION_DATA_ACTION_URL, notify->link }
						}
					}
				}
            }
...
)
{
		...
		{ JSON_NOTIFICATION, 
			{
				...
				{ JSON_NOTIFICATION_DATA, // new one is object
					{ JSON_NOTIFICATION_DATA_ACTION_DEFAULT,
						{
							{ JSON_NOTIFICATION_DATA_ACTION_OPERATION, "openWindow" },
							{ JSON_NOTIFICATION_DATA_ACTION_URL, link }
						}
					}
				}
			}
	...
	if (!badge.empty())
		n[JSON_NOTIFICATION_BADGE] = badge;
	if (!sound.empty())
		n[JSON_NOTIFICATION_SOUND] = sound;
	if (!link.empty()) {
		n[JSON_NOTIFICATION_LINK] = link;
	}
	if (!data.empty()) {
		// deprecated
		n[JSON_NOTIFICATION_EXTRA_DATA] = data;
	}
	...
	if (r == 0)
	{
		...
		try {
			...
			if (n.count(JSON_NOTIFICATION_ICON))
				...
				icon = "";
			if (n.count(JSON_NOTIFICATION_BADGE))
				badge = n.at(JSON_NOTIFICATION_BADGE);
			else
				badge = "";
			if (n.count(JSON_NOTIFICATION_SOUND))
				sound = n.at(JSON_NOTIFICATION_SOUND);
			else
				sound = "";
			...
			if (n.count(JSON_NOTIFICATION_DATA)) {
				JsonValue dat = n.at(JSON_NOTIFICATION_DATA);
				if (dat.is_string())
					data = dat;
				else {
					if (dat.is_object()) {
						if (dat.count(JSON_NOTIFICATION_DATA_ON_ACTION_CLICK)) {
							JsonValue oac = dat.at(JSON_NOTIFICATION_DATA_ON_ACTION_CLICK);
							if (oac.is_object()) {
								if (oac.count(JSON_NOTIFICATION_DATA_ACTION_DEFAULT)) {
									JsonValue defaultAction = oac.at(JSON_NOTIFICATION_DATA_ACTION_DEFAULT);
									if (defaultAction.is_object()) {
										if (defaultAction.count(JSON_NOTIFICATION_DATA_ACTION_URL)) {
											JsonValue url = defaultAction.at(JSON_NOTIFICATION_DATA_ACTION_URL);
											if (url.is_string()) {
												click_action = url;
											}
										}
