	...
	case kDataMessageStanzaTag:
	{
		...
		*/
		...
		if (r->has_raw_data())
		{
			...
			if (dr == 0)
			{
				if (client->verbosity > 3) {
					std::cerr << "Payload: " << d << std::endl;
				}
				std::string appName;
				std::string appId;
...
)
{
	...
	int r = parseNotificationJson(value, retval.authorizedEntity, toEntity, 
		retval.title, retval.body, retval.icon, retval.badge, retval.sound, retval.link, retval.data);
	return r;
}
...
static void copyRefNotifyMessage(NotifyMessageC *retval, const NotifyMessage *value)
{
	...
	retval->badge = value->badge.c_str();
